...
function renderDiscounts() {
  ...
  list.forEach((d) => {
    d._effStatus = !d.active
      ? "Inactive"
      : d.end && new Date(d.end) < today
        ? "Expired"
        : "Active";
  });
  ...
  if (!list.length) {
    tbody.innerHTML =
      '<tr><td colspan="8" style="text-align:center;padding:40px;color:var(--text-soft)">No discounts found.</td></tr>';
    return;
  }
    ...
    <tr>
      ...
      <td>
        <div style="display:flex;gap:4px">
         <button type="button" class="btn btn-outline btn-icon" onclick="toggleDiscountStatus('${d.code}')">
            <svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
          </button>
         <button type="button" class="btn btn-danger btn-icon" onclick="deleteDiscount('${d.code}')">
...
function openAddDiscount() {
  ...
  clearDiscountForm();
  document.getElementById("discount-modal-title").textContent =
    "Create Discount";
...
function saveDiscount() {
  ...
  if (!code) {
    showToast("⚠ Discount code is required");
    return;
  }
  const d = {
    code,
    ...
    limit: document.getElementById("disc-limit")?.value
      ? +document.getElementById("disc-limit").value
      : null,
    uses: 0,
    start:
      document.getElementById("disc-start")?.value ||
  ...
  if (editingDiscCode) {
    ...
    if (i > -1) {
      d.uses = SC_DISCOUNTS[i].uses;
      SC_DISCOUNTS[i] = d;
    }
  } else {
    if (SC_DISCOUNTS.find((x) => x.code === code)) {
...
function deleteDiscount(code) {
  ...
  window.location.href = "discounts.php?delete=" + code;
}
/* ── TOGGLE STATUS ───────────────────────── */

function toggleDiscountStatus(code) {
  if (!confirm("Change discount status?")) return;

  window.location.href = "discounts.php?toggle=" + code;
}
/* ── INIT ────────────────────────────────────────────────── */
