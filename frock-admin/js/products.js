...
function renderProducts() {
  ...
  /* Update tab counts */
  document.getElementById("tab-all-count").textContent = (
    SC_PRODUCTS || []
  ).length;
  document.getElementById("tab-active-count").textContent = (
    SC_PRODUCTS || []
  ).filter((p) => p.status === "Active").length;
  document.getElementById("tab-hold-count").textContent = (
    SC_PRODUCTS || []
  ).filter((p) => p.status === "On Hold").length;
  document.getElementById("tab-draft-count").textContent = (
    SC_PRODUCTS || []
...
function bulkAction(action) {
  const checked = [...document.querySelectorAll(".prod-cb:checked")].map(
    (c) => c.dataset.id,
  );
  if (!checked.length) {
  ...
  checked.forEach((id) => {
    ...
    if (action === "remove") {
      const i = SC_PRODUCTS.indexOf(p);
      if (i > -1) SC_PRODUCTS.splice(i, 1);
    }
  });
  showToast(
    action === "remove"
      ? `🗑 ${checked.length} product(s) removed`
      : `✓ ${checked.length} product(s) updated`,
  );
  renderProducts();
}
